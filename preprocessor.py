...
class TextPreprocessor:
    def __init__(
        self, cleaner, input_path=".", output_path=".", max_length=600, with_labels=True
    ):
    ...
    def clean_lines(self, lines, data_type):
        ...
        if self.with_labels:
            with open(
                f"{self.output_path}cleaned_{data_type}_with_diacritics.txt",
                "a+",
                encoding="utf-8",
            ) as f:
                f.write("\n".join(lines) + "\n")
        ...
        with open(
            f"{self.output_path}cleaned_{data_type}_without_diacritics.txt",
            "a+",
            encoding="utf-8",
        ) as f:
